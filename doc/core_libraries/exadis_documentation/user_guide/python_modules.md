...
```python
G = ExaDisNet()
G.generate_line_config(crystal, Lbox, num_lines, theta, maxseg, seed)
```
where `crystal` is the crystal structure, `Lbox` is the simulation cell size, `numsources`/`num_lines` the number of loops/lines to generate, `maxseg` the maximum segment size to discretize the dislocations, `seed` the seed number for the random generation. For loops, the dislocations are generated by cycling through the list of native Burgers vectors. For lines/dipoles, the dislocations are generated by cycling through the list of signed slip systems (+/- Burgers vectors). To create dipoles (neutral Burgers charge), `num_lines` must be a multiple of 24. If a list of character angles `theta` is provided, each dislocation will be randomly assigned one of the character angles from the list. If not provided, the character angles will be chosen such that the dislocation density is roughly equal between all slip systems.
...
- `ExaDisNet.generate_line_config(crystal, Lbox, num_lines, theta, maxseg, seed)`: Set the content of the `ExaDisNet` object by generating a configuration made of straight, infinite lines/dipoles.
